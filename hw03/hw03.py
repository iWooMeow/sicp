...
def num_eights(pos):
    ...
    if pos < 10:
        ...
        return num_eights(pos // 10) + (pos % 10 == 8) + 0
...
def pingpong(n):
    ...
    def helper(result, index, dir):
        if index == n:
            return result
        elif index % 8 == 0 or num_eights(index) > 0:
            return helper(result - dir, index + 1, -dir)
        else:
            return helper(result + dir, index + 1, dir)
...
def count_coins(change):
    ...
    def helper(change, largest):
        if largest == 1:
            ...
            # withLargest = count_coins(change - n)
            # withoutLargest = count_coins(change)
            # change -= n
            return helper(change, next_smaller_coin(largest)) + helper(
...
def beaver(f):
    "*** YOUR CODE HERE ***"
    return (lambda g:lambda x:g(g(g(g(g(g(g(x))))))))(lambda g:g(g(g(g(f))))) (lambda f :lambda : f())()

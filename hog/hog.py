...
GOAL = 1000000  # The goal of Hog is to score 100 points.
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    lastDigit = opponent_score % 10
    secondDigit = (opponent_score // 10) % 10
    return 2 * abs(lastDigit - secondDigit) + 1
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while a < goal:
        b = 0
        while b < goal:
            if strategy(a, b) == roll and status == True:
                status = True
            else:
                status = False
            b = b + 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while times <= 10:
        ...
        result = averaged_dice(times, dice)
        if result > resultMax:
            resultMax = result
            timeMax = times
        times = times + 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    presentScore = square_update(0, score, opponent_score)
    if (presentScore - score) > threshold:
        num_rolls = 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    baseline = 8.72
    presentScore = square_update(0, score, opponent_score)
    avr = make_averaged(roll_dice, 100000)
    avr1 = 3.5
    avr2 = 5.86
    avr3 = 7.36
    avr4 = 8.2
    avr5 = 8.6
    if (presentScore - score) > baseline:
        return 0
    elif tail_points(opponent_score) > baseline:
        return 0
    elif avr1 > (100 - score):
        return 1
    elif avr2 > (100 - score):

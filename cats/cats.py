...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valids = [element for element in paragraphs if select(element)]
    if len(valids) - 1 < k:
        return ""
    else:
        return valids[k]
...
def about(topic):
    ...
    def inputString(string):
        ...
        string = lower(remove_punctuation(string))
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for source in word_list:
            actualDiff = diff_function(typed_word, source, limit)
            if actualDiff <= limit and min > actualDiff:
                min = actualDiff
                index = count
            count += 1
        if index < 0:
...
def feline_fixes(typed, source, limit):
    ...
    # count = min(len(typed), len(source))
    # sub = abs(len(source) - len(typed))
    #
    # def helper(curr, times):
    #     if curr > limit:
    #         return curr
    #     elif times == -1:
    #         return curr
    #     else:
    #         return helper(curr + (typed[times] != source[times]), times - 1)
    #
    # return helper(sub, count - 1)
    ...
    # return 0
        ...
        return (typed[0] != source[0]) + feline_fixes(
...
def final_diff(typed, source, limit):
    ...
    if limit < 0:
        return 0
    # elif typed in mis and source == mis[typed]:
    #     return 1
    elif typed == "" or source == "":
        return abs(len(source) - len(typed))
    # elif typed[:2] == source[:2:-1]:
    #     return final_diff(typed[2:], source[2:], limit - 1) + 1
    # elif source[:2] == source[:2:-1] and (typed[:2] != typed[:2:-1]):
    #     return final_diff(typed[2:], source[2:], limit - 1) + 1
    elif typed[0] == source[0]:
        return final_diff(typed[1:], source[1:], limit)

    else:
        add = final_diff(typed, source[1:], limit - 1) + 1  # Fill in these lines
    ...
    #     substitute = minimum_mewtations(typed[2:], source[1:], limit - 1) + 1
    #     # BEGIN
    #     # "*** YOUR CODE HERE ***"
    #     return min(add, remove, substitute)
    # END
...
def fastest_words(match):
    ...
    #     ans.append([])
    # fill the number of players []
    for j in word_indices:
        # minPlayer = 0
        # for i in player_indices:
        #     if get_all_times(match)[i][j] < get_all_times(match)[minPlayer][j]:
        #         minPlayer = i
        minPlayer = min(player_indices, key=lambda x: get_all_times(match)[x][j])
        ans[minPlayer].append(get_all_words(match)[j])
    return ans

def flatten(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return [element if type(element) != list else flatten(element) for element in s]
    # tmp = []
    # for element in s:
    #     if type(element) != list:
    #         tmp.append(element)
    #     else:
    #         tmp.extend(flatten(element))
    # return tmp
    if not s:
        return []
    elif type(s[0]) == list:
...
def sprout_leaves(t, leaves):
    ...
    return tree(
        ...
        [
            ...
            if is_leaf(branch) == False
            else tree(label(branch), [tree(leaf) for leaf in leaves])
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return [label(b) if is_leaf(b) else   for b in branches(t)]
    # ans = []
    # ans.append(label(t))
    # for b in branches(t):
    #     if is_leaf(b):
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):
        ...
        branchtmp = branches(tmpTree)
        branchtmp.extend(branches(t2)[len(branches(t1)) :])
        ...
        return tree(
            label(tmpTree),
            branches(tmpTree) + branches(t1)[len(branches(t2)) :],
        )
